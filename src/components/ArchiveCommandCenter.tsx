...
export function ArchiveCommandCenter({
  ...
  const [activeTab, setActiveTab] = useState<'directory' | 'compare' | 'quiz' | 'profile'>('directory');
  ...
  const [unlockedStamps, setUnlockedStamps] = useState<string[]>(['centrifuge']); // default entry stamp
  ...
  useEffect(() => {
    const handleAddRecentVisit = (e: Event) => {
      ...
      if (customEvt.detail) {
        trackVisit(customEvt.detail.type, customEvt.detail.id, customEvt.detail.label);
      }
    };
  ...
  const trackVisit = (type: RecentVisit['type'], id: string | number, label: string) => {
    ...
    setRecentVisits(prev => {
      ...
      try {
        localStorage.setItem('wc_command_recent_visits', JSON.stringify(updated));
  ...
  const toggleFavorite = (type: FavoriteItem['type'], id: string | number, label: string, e?: React.MouseEvent) => {
  ...
  const handleItemNavigation = (type: SearchItem['type'], id: string | number, label: string) => {
  ...
  const stampDefs = useMemo<StampDef[]>(() => [
    { id: 'centrifuge', title: 'GOLDEN WATERMARK', subtitle: 'Command Access', req: 'Unlocked upon entering the Archives', unlocked: unlockedStamps.includes('centrifuge'), icon: '🎟️', color: 'border-[#D4AF37] text-[#D4AF37]' },
    { id: 'albiceleste_pride', title: 'ALBICELESTE CORONATION', subtitle: 'Argentina Vault', req: 'Explore Argentina in the Nations Vault', unlocked: unlockedStamps.includes('albiceleste_pride'), icon: '🇦🇷', color: 'border-sky-400 text-sky-400' },
    { id: 'pele_samba', title: '3-STAR KINGDOM', subtitle: 'Pelé Legacy Stamp', req: 'View the Legendary Pelé profile', unlocked: unlockedStamps.includes('pele_samba'), icon: '👑', color: 'border-yellow-400 text-yellow-400' },
    { id: 'maracanazo_witness', title: 'MARACANAZO MEMOIR', subtitle: '1950 World Cup Stamp', req: 'Explore the 1950 World Cup history', unlocked: unlockedStamps.includes('maracanazo_witness'), icon: '🇺🇾', color: 'border-blue-400 text-blue-400' },
    { id: 'azteca_cathedral', title: 'THE AZTECA ECHO', subtitle: 'Stadium Temple Stamp', req: 'Visit Estadio Azteca in Stadiums', unlocked: unlockedStamps.includes('azteca_cathedral'), icon: '🏟️', color: 'border-amber-500 text-amber-500' },
    { id: 'trivia_adept', title: 'GRAND TRIVIA SEAL', subtitle: 'Quiz Arena Victor', req: 'Perform a flawless trivia session', unlocked: unlockedStamps.includes('trivia_adept'), icon: '🎖️', color: 'border-emerald-400 text-emerald-400' },
    { id: 'curator_license', title: 'CURATOR COMMUNIQUE', subtitle: 'Duel Analyst Badge', req: 'Conduct a Dual Comparison Lab search', unlocked: unlockedStamps.includes('curator_license'), icon: '⚓', color: 'border-purple-400 text-purple-400' },
  ], [unlockedStamps]);
  ...
  const lastStateCurations = useMemo(() => {
    ...
    return {
      tournament: tournamentVisit ? { id: tournamentVisit.id, label: tournamentVisit.label } : { id: 2022, label: '2022 Qatar' },
      legend: legendVisit ? { id: legendVisit.id, label: legendVisit.label } : { id: 'pele', label: 'Pelé (Brazil)' },
      nation: nationVisit ? { id: nationVisit.id, label: nationVisit.label } : { id: 'argentina', label: 'Argentina' },
      stadium: stadiumVisit ? { id: stadiumVisit.id, label: stadiumVisit.label } : { id: 'maracana', label: 'Maracanã' },
      match: matchVisit ? { id: matchVisit.id, label: matchVisit.label } : { id: '2022-final', label: 'Argentina 3-3 France (2022)' }
    };
  }, [recentVisits]);
  ...
  const recommendedJourneys = useMemo(() => {
    ...
    if (lastLegendVisit && String(lastLegendVisit.id) === 'maradona') {
      return [
        { type: 'tournament' as const, id: 1986, label: 'Mexico 1986 Tournament', icon: '🏆', desc: 'Step back to Argentina\'s pinnacle victory' },
        { type: 'match' as const, id: '1986-quarterfinal', label: 'Hand of God Duel', icon: '⚽', desc: 'The most controversial game in history' },
        { type: 'stadium' as const, id: 'azteca', label: 'Estadio Azteca', icon: '🏟️', desc: 'The stage of Maradona\'s Goal of the Century' }
      ];
    }
    // Default Iconic Recommendations
    return [
      { type: 'tournament' as const, id: 1970, label: 'Brazil 1970 Dynasty', icon: '🏆', desc: 'The finest collective exhibition of collective football' },
      { type: 'legend' as const, id: 'pele', label: 'Pelé Heritage', icon: '👑', desc: 'The King\'s classic records and vintage exhibits' },
      { type: 'match' as const, id: '2022-final', label: '2022 Coronation Final', icon: '⚽', desc: 'Explore the definitive, dramatic final in Lusail' }
    ];
  }, [recentVisits]);
  ...
  const quizQuestions = useMemo<QuizQuestion[]>(() => [
    {
      id: 1,
      question: "Which iconic player stands alone in historical annals as the owner of three FIFA World Cup titles?",
      options: ["Pelé (Brazil)", "Diego Maradona (Argentina)", "Franz Beckenbauer (Germany)", "Garrincha (Brazil)"],
      correctAnswer: "Pelé (Brazil)",
      explanation: "Pelé won the championship in 1958, 1962, and 1970, confirming his crown in footballing folklore."
    },
    {
      id: 2,
      question: "In which tournament did the legendary shock nicknamed 'The Maracanazo' shatter the host nation's dreams?",
      options: ["France 1938", "Brazil 1950", "Switzerland 1954", "Chile 1962"],
      correctAnswer: "Brazil 1950",
      explanation: "Uruguay defeated host nation Brazil 2-1 on July 16, 1950, in front of a staggering 200,000 spectators."
    },
    {
      id: 3,
      question: "Which conceptual tactical system was brought to revolutionized glory by Johan Cruyff's Netherlands in 1974?",
      options: ["Catenaccio Defensive Block", "Total Football (Totaalvoetbal)", "Tiki-Taka Pass Harmony", "Gegenpressing Counter-Blitz"],
      correctAnswer: "Total Football (Totaalvoetbal)",
      explanation: "Total Football allowed players to switch roles fluidly across positions, creating beautiful, coordinated football choreography."
    },
    {
      id: 4,
      question: "In the dramatic 3-3 Lusail final of 2022, who registered a relentless hat-trick for France to keep them in contention?",
      options: ["Antoine Griezmann", "Olivier Giroud", "Kylian Mbappé", "Zinedine Zidane"],
      correctAnswer: "Kylian Mbappé",
      explanation: "Kylian Mbappé scored two quick penalties and a classic high-intensity volley to secure his stunning final hat-trick."
    }
  ], []);
  ...
  const legendsComparison = useMemo(() => {
    ...
    const statsMap: Record<string, { titles: number; goals: number; matches: number; longevityScore: number; rating: number }> = {
      pele: { titles: 3, goals: 12, matches: 14, longevityScore: 92, rating: 99 },
      maradona: { titles: 1, goals: 8, matches: 21, longevityScore: 88, rating: 98 },
      cruyff: { titles: 0, goals: 3, matches: 7, longevityScore: 85, rating: 96 },
      ronaldo: { titles: 2, goals: 15, matches: 19, longevityScore: 91, rating: 97 },
      zidane: { titles: 1, goals: 5, matches: 12, longevityScore: 90, rating: 97 }
    };

    const statsA = statsMap[defaultA.id] || { titles: 1, goals: 4, matches: 10, longevityScore: 85, rating: 95 };
    const statsB = statsMap[defaultB.id] || { titles: 1, goals: 4, matches: 10, longevityScore: 85, rating: 95 };

    // Verdict editorial dynamic synthesis
    let verdictStr = '';
    if (defaultA.id === 'pele' && defaultB.id === 'maradona') {
      verdictStr = "COLLECTIVE KING VS MASTER OF FORCE. Pelé represents the absolute collective pinnacle of a dominant golden Brazilian generation, claiming 3 stars. Diego Maradona represents the ultimate peak of individual orchestration, famously bending Mexico '86 into his sole personal canvas. A deadlock between system supremacy and artistic rebellion.";
    } else if (defaultA.id === 'pele' && defaultB.id === 'ronaldo') {
      verdictStr = "THE KINGS OF SAMBA. Pelé remains the original teenager who took Sweden by storm and established Brazil's global sovereignty. Ronaldo Nazário conquered horrific knee traumas for his iconic 2002 redemptive double. Both stand as absolute symbols of Brazilian goalscoring flair.";
    } else {
  ...
  const handleNextQuestion = () => {
    ...
    if (currentIdx + 1 < quizQuestions.length) {
      ...
      if (score + (selectedOpt === quizQuestions[currentIdx].correctAnswer ? 1 : 0) === quizQuestions.length) {
  ...
  const resetVaultProgress = () => {
    if (confirm('Are you sure you want to reset your Archive Explorer progression? This will clear your XP, unlocked stamps, and recently viewed contents.')) {
      setXp(100);
      ...
      try {
        ...
        localStorage.setItem('wc_command_stamps', JSON.stringify(['centrifuge']));
            ...
            <div className="px-6 md:px-12 py-5 flex items-center justify-between z-10 select-none">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-[9px] uppercase tracking-[0.25em] text-[#D4AF37] opacity-80">Museum Control Room</span>
                    <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span>
                  </div>
                  <h1 className="font-serif text-[#F5F2EA] text-xl tracking-[0.15em] uppercase font-bold leading-none">ARCHIVE COMMAND CENTER</h1>
                </div>
              </div>
              ...
              <div className="hidden lg:flex items-center bg-[#111111] border border-white/5 p-1 rounded-[3px] gap-1 font-mono text-[10px] tracking-wider uppercase">
                ...
                <button
                  ...
                  onClick={() => { setActiveTab('compare'); triggerCompareLabUnlock(); }}
                >
                  ⚔️ Comparison Lab
                </button>
                <button
              ...
              <div className="flex items-center gap-3">
                <span className="hidden sm:inline font-mono text-[9px] text-[#69707A] uppercase tracking-widest bg-white/5 px-2.5 py-1 border border-white/5 rounded-none">ESC / CLOSE</span>
                <button
              ...
              <div className="lg:col-span-4 flex flex-col gap-6">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-0.5">
                        <span className="font-sans font-semibold text-sm tracking-wide text-[#F5F2EA] truncate">{profileDetails.title}</span>
                      </div>
                      <p className="font-mono text-[9px] uppercase tracking-wider text-[#69707A]">World Cup Vault Explorer</p>
                ...
                <div className="bg-[#0b0b0b] border border-[#4E5661]/15 p-6 rounded-none shadow-lg">
                  <div className="flex items-center gap-2 mb-4 border-b border-white/5 pb-2.5">
                    ...
                    <h3 className="font-serif text-[#F5F2EA] text-xs uppercase tracking-[0.2em] font-semibold">Continue Your Journey</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-2.5">
                    ...
                    <button
                      ...
                      onClick={() => handleItemNavigation('tournament', lastStateCurations.tournament.id, lastStateCurations.tournament.label)}
                    >
                      <div className="min-w-0">
                        <span className="block font-mono text-[8px] uppercase tracking-wider text-[#69707A] mb-0.5">Last Explored Year</span>
                        <span className="font-sans text-xs text-[#AFA58D] font-medium group-hover:text-white transition-all truncate block">{lastStateCurations.tournament.label}</span>
                      </div>
                    ...
                    <button
                      className="bg-[#121212] hover:bg-[#1a1a1a] border border-white/5 p-3 rounded-none text-left flex items-center justify-between transition-all group cursor-pointer"
                      onClick={() => handleItemNavigation('legend', lastStateCurations.legend.id, lastStateCurations.legend.label)}
                    >
                      <div className="min-w-0">
                        <span className="block font-mono text-[8px] uppercase tracking-wider text-[#69707A] mb-0.5">Last Legend Opened</span>
                        <span className="font-sans text-xs text-[#AFA58D] font-medium group-hover:text-white transition-all truncate block">{lastStateCurations.legend.label}</span>
                      </div>
                      <ChevronRight className="w-4 h-4 text-[#D4AF37] opacity-60 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all flex-shrink-0 ml-2" />
                    </button>
                    ...
                    <button
                      className="bg-[#121212] hover:bg-[#1a1a1a] border border-white/5 p-3 rounded-none text-left flex items-center justify-between transition-all group cursor-pointer"
                      onClick={() => handleItemNavigation('nation', lastStateCurations.nation.id, lastStateCurations.nation.label)}
                    >
                      <div className="min-w-0">
                        <span className="block font-mono text-[8px] uppercase tracking-wider text-[#69707A] mb-0.5">Last Nation Searched</span>
                        <span className="font-sans text-xs text-[#AFA58D] font-medium group-hover:text-white transition-all truncate block">{lastStateCurations.nation.label}</span>
                      </div>
                      <ChevronRight className="w-4 h-4 text-[#D4AF37] opacity-60 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all flex-shrink-0 ml-2" />
                    </button>
                    ...
                    <button
                      className="bg-[#121212] hover:bg-[#1a1a1a] border border-white/5 p-3 rounded-none text-left flex items-center justify-between transition-all group cursor-pointer"
                      onClick={() => handleItemNavigation('stadium', lastStateCurations.stadium.id, lastStateCurations.stadium.label)}
                    >
                      <div className="min-w-0">
                        <span className="block font-mono text-[8px] uppercase tracking-wider text-[#69707A] mb-0.5">Last Stadium Visited</span>
                        <span className="font-sans text-xs text-[#AFA58D] font-medium group-hover:text-white transition-all truncate block">{lastStateCurations.stadium.label}</span>
                      </div>
                      <ChevronRight className="w-4 h-4 text-[#D4AF37] opacity-60 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all flex-shrink-0 ml-2" />
                    </button>
                  </div>
                ...
                <div className="bg-[#0b0b0b] border border-[#4E5661]/15 p-6 rounded-none shadow-lg">
                  <div className="flex items-center gap-2 mb-4 border-b border-white/5 pb-2.5">
                    ...
                    <h3 className="font-serif text-[#F5F2EA] text-xs uppercase tracking-[0.2em] font-semibold">Saved Collections</h3>
                  </div>
                    ...
                    <div className="max-h-[140px] overflow-y-auto space-y-1.5 scrollbar-thin scrollbar-thumb-white/10 pr-1 select-none">
                      {favorites.map((fav, i) => (
                        <div 
                          ...
                          onClick={() => handleItemNavigation(fav.type, fav.id, fav.label)}
                        >
                          ...
                          <button
                            ...
                            onClick={(e) => { e.stopPropagation(); toggleFavorite(fav.type, fav.id, fav.label); }}
                            title="Remove pin"
                          >
                  ...
                  <div className="flex flex-col gap-6">
                        ...
                        <div className="absolute left-0 right-0 top-full mt-2 bg-[#090909] border border-[#D4AF37]/35 shadow-2.5xl max-h-[300px] overflow-y-auto z-[200] divide-y divide-white/[0.03] select-none">
                          {filteredSearchItems.length === 0 ? (
                            <div className="p-6 text-center text-[#69707A] text-xs">
                              No historic entries found matching and cataloged under "{searchQuery}".
                            </div>
                          ) : (
                            filteredSearchItems.map((item, idx) => (
                              <div
                                ...
                                onClick={() => handleItemNavigation(item.type, item.id, item.label)}
                              >
                                <div className="min-w-0 pr-4">
                                  <div className="flex items-center gap-2 mb-0.5">
                                    ...
                                    <span className="font-mono text-[9px] text-[#69707A]">#{item.id}</span>
                                  </div>
                                  <h4 className="font-serif text-[#F5F2EA] text-sm group-hover:text-[#D4AF37] transition-all leading-snug truncate whitespace-nowrap">{item.title}</h4>
                                  <p className="font-sans text-xs text-[#AFA58D]/70 truncate max-w-lg leading-normal block">{item.subtitle}</p>
                                </div>
                                <div className="flex items-center gap-3">
                                  <button
                                    ...
                                    onClick={(e) => toggleFavorite(item.type, item.id, item.label, e)}
                                    title={isFavorited(item.type, item.id) ? "Remove Bookmark" : "Bookmark Entry"}
                                  >
                                    <Star className={`w-3.5 h-3.5 ${isFavorited(item.type, item.id) ? 'fill-[#D4AF37] text-[#D4AF37]' : ''}`} />
                                  </button>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 select-none">
                      ...
                      <div className="bg-[#0b0b0b] border border-[#4E5661]/15 p-5 rounded-none flex flex-col justify-between">
                        <div>
                          ...
                          <p className="font-sans text-xs text-[#AFA58D] leading-relaxed mb-4">
                            Step into large historical chronicles and geographic representations mapping the triumphs of the beautiful game.
                          </p>
                        </div>
                        <div className="space-y-2">
                          <button 
                            ...
                            onClick={() => { onExploreHistory(); setIsOpen(false); }}
                          >
                            <div className="flex items-center gap-2.5">
                              <span className="text-sm">🧭</span>
                              <span className="font-sans text-xs text-[#F5F2EA] hover:text-white transition-all font-medium">History Journey</span>
                            </div>
                          ...
                          <button 
                            className="w-full text-left bg-[#121212] hover:bg-[#D4AF37]/5 border border-white/5 p-3 rounded-none flex items-center justify-between group transition-all cursor-pointer"
                            onClick={() => { onExploreHistory(); setIsOpen(false); }} // mapped to time machine
                          >
                            <div className="flex items-center gap-2.5">
                              <span className="text-sm">⏳</span>
                              <span className="font-sans text-xs text-[#F5F2EA] hover:text-white transition-all font-medium">World Cup Time Machine</span>
                            </div>
                            <ChevronRight className="w-3.5 h-3.5 text-[#F5F2EA]/40 group-hover:text-[#D4AF37] group-hover:translate-x-0.5 transition-all" />
                          </button>
                          ...
                          <button 
                            className="w-full text-left bg-[#121212] hover:bg-[#D4AF37]/5 border border-white/5 p-3 rounded-none flex items-center justify-between group transition-all cursor-pointer"
                            onClick={() => { onExploreAtlas(); setIsOpen(false); }}
                          >
                            <div className="flex items-center gap-2.5">
                              <span className="text-sm">🗺️</span>
                              <span className="font-sans text-xs text-[#F5F2EA] hover:text-white transition-all font-medium">Football Atlas Explorer</span>
                            </div>
                            <ChevronRight className="w-3.5 h-3.5 text-[#F5F2EA]/40 group-hover:text-[#D4AF37] group-hover:translate-x-0.5 transition-all" />
                          </button>
                        </div>
                      ...
                      <div className="bg-[#0b0b0b] border border-[#4E5661]/15 p-5 rounded-none flex flex-col justify-between">
                        <div>
                          ...
                          <p className="font-sans text-xs text-[#AFA58D] leading-relaxed mb-4">
                            Direct queries into curated collections cataloged by stadiums, historic matches, and legendary individuals.
                          </p>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <button 
                            onClick={() => { onExploreLegends(); setIsOpen(false); }}
                          ...
                          <button 
                            onClick={() => { onExploreNations(); setIsOpen(false); }}
                            className="bg-[#121212] hover:bg-[#D4AF37]/5 border border-white/5 p-2 rounded-none text-center block transition-all text-[11px] font-medium text-[#AFA58D] hover:text-white cursor-pointer"
                          >
                            🏳️ Nations Vault
                          </button>
                          <button 
                            onClick={() => { onExploreMatches(); setIsOpen(false); }}
                            className="bg-[#121212] hover:bg-[#D4AF37]/5 border border-white/5 p-2 rounded-none text-center block transition-all text-[11px] font-medium text-[#AFA58D] hover:text-white cursor-pointer"
                          >
                            ⚽ Matches Vault
                          </button>
                          <button 
                            onClick={() => { onExploreStadiums(); setIsOpen(false); }}
                            className="bg-[#121212] hover:bg-[#D4AF37]/5 border border-white/5 p-2 rounded-none text-center block transition-all text-[11px] font-medium text-[#AFA58D] hover:text-white cursor-pointer"
                          >
                            🏟️ Stadiums Vault
                          </button>
                          <button 
                            onClick={() => { onExploreRecords(); setIsOpen(false); }}
                    ...
                    <div className="bg-[#0b0b0b] border border-[#4E5661]/15 p-5 shadow-lg select-none">
                      <div className="flex items-center gap-2 mb-3 border-b border-white/5 pb-2">
                        ...
                        <h4 className="font-serif text-[#F5F2EA] text-xs uppercase tracking-[0.2em] font-semibold">Recommended Discovery Paths</h4>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
                        {recommendedJourneys.map((rec, idx) => (
                          <div 
                            ...
                            onClick={() => handleItemNavigation(rec.type, rec.id, rec.label)}
                          >
                            <div>
                              <div className="flex items-center justify-between mb-1">
                                <span className="font-mono text-[7px] uppercase tracking-widest text-[#D4AF37] bg-[#D4AF37]/10 px-1.5 py-0.5 leading-none">Suggested</span>
                                <span className="text-xs">{rec.icon}</span>
                              </div>
                              <h5 className="font-serif text-[#F5F2EA] text-xs font-semibold group-hover:text-[#D4AF37] transition-all leading-snug">{rec.label}</h5>
                              <p className="font-sans text-[10px] text-[#69707A] leading-normal mt-1">{rec.desc}</p>
                            </div>
                    ...
                    <div className="bg-[#0b0b0b] border border-[#4E5661]/15 p-5 shadow-lg select-none">
                      <div className="flex items-center gap-2 mb-3 border-b border-white/5 pb-2">
                        ...
                        <h4 className="font-serif text-[#F5F2EA] text-xs uppercase tracking-[0.2em] font-semibold">Instant Tournament Selector</h4>
                      </div>
                      <div className="flex flex-wrap gap-1.5 max-h-[140px] overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 pr-1 select-none">
                        {tournaments.slice().reverse().map((t) => (
                          <button
                            key={t.year}
                            onClick={() => handleItemNavigation('tournament', t.year, `${t.year} World Cup`)}
                ...
                {activeTab === 'compare' && (
                  <div className="bg-[#0b0b0b] border border-[#4E5661]/15 p-6 rounded-none shadow-xl flex flex-col gap-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-white/5 pb-4 select-none">
                      <div>
                        <h3 className="font-serif text-[#F5F2EA] text-base uppercase tracking-widest font-bold">⚔️ COMPARISON LAB</h3>
                        <p className="font-sans text-xs text-[#69707A] leading-normal mt-0.5">Calculate dual stats and simulated clashes between historical icons.</p>
                      </div>
                    ...
                    {compareType === 'legend' ? (
                      <div className="grid grid-cols-2 gap-4 select-none">
                        ...
                        <div className="flex flex-col">
                          <label className="font-mono text-[9px] uppercase tracking-widest text-[#69707A] mb-1.5">LEGEND INDEX A</label>
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.value === compareLegendB) {
                                // set to other
                                setCompareLegendB(legends.find(l => l.id !== e.target.value)?.id || 'maradona');
                              }
                            }}
                          >
                            {legends.map(l => (
                              <option key={l.id} value={l.id}>{l.name} ({l.nation})</option>
                            ))}
                          </select>
                        </div>
                        {/* Selector Legend B */}
                        <div className="flex flex-col">
                          <label className="font-mono text-[9px] uppercase tracking-widest text-[#69707A] mb-1.5">LEGEND INDEX B</label>
                          <select
                            className="bg-[#121212] border border-[#4E5661]/15 focus:border-[#D4AF37]/50 p-2.5 rounded-none text-xs text-[#F5F2EA]"
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.value === compareLegendA) {
                                setCompareLegendA(legends.find(l => l.id !== e.target.value)?.id || 'pele');
                              }
                            }}
                          >
                            {legends.map(l => (
                      ...
                      <div className="grid grid-cols-2 gap-4 select-none">
                        {/* Selector Nation A */}
                        <div className="flex flex-col">
                          <label className="font-mono text-[9px] uppercase tracking-widest text-[#69707A] mb-1.5">NATION CIVILIZATION A</label>
                          <select
                            className="bg-[#121212] border border-[#4E5661]/15 focus:border-[#D4AF37]/50 p-2.5 rounded-none text-xs text-[#F5F2EA]"
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.value === compareNationB) {
                                setCompareNationB(nationsData.find(n => n.id !== e.target.value)?.id || 'brazil');
                              }
                            }}
                          >
                            {nationsData.map(n => (
                        ...
                        {/* Selector Nation B */}
                        <div className="flex flex-col">
                          <label className="font-mono text-[9px] uppercase tracking-widest text-[#69707A] mb-1.5">NATION CIVILIZATION B</label>
                          <select
                            className="bg-[#121212] border border-[#4E5661]/15 focus:border-[#D4AF37]/50 p-2.5 rounded-none text-xs text-[#F5F2EA]"
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.value === compareNationA) {
                                setCompareNationA(nationsData.find(n => n.id !== e.target.value)?.id || 'argentina');
                              }
                            }}
                          >
                            {nationsData.map(n => (
                    ...
                    {compareType === 'legend' ? (
                      <div className="flex flex-col gap-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 select-none">
                          <div className="bg-[#121212] border border-[#4E5661]/10 p-5 rounded-none flex items-start gap-4">
                            <img 
                              ...
                              className="w-14 h-14 object-cover filter grayscale contrast-125 rounded-none border border-[#D4AF37]/25"
                              referrerPolicy="no-referrer"
                            />
                            <div>
                              <span className="font-mono text-[8px] bg-[#D4AF37]/10 text-[#D4AF37] px-2 py-0.5 leading-none">DEITY A</span>
                              <h4 className="font-serif text-[#F5F2EA] text-sm font-semibold truncate leading-snug mt-1">{legendsComparison.legendA.name}</h4>
                              <p className="font-mono text-[9px] text-[#AFA58D]">{legendsComparison.legendA.nation}</p>
                              <p className="font-sans text-[10px] italic text-[#69707A] mt-2 block line-clamp-2">"{legendsComparison.legendA.quote}"</p>
                            </div>
                          </div>
                          ...
                          <div className="bg-[#121212] border border-[#4E5661]/10 p-5 rounded-none flex items-start gap-4">
                            ...
                            />
                            <div>
                              <span className="font-mono text-[8px] bg-[#D4AF37]/10 text-[#D4AF37] px-2 py-0.5 leading-none">DEITY B</span>
                              <h4 className="font-serif text-[#F5F2EA] text-sm font-semibold truncate leading-snug mt-1">{legendsComparison.legendB.name}</h4>
                              <p className="font-mono text-[9px] text-[#AFA58D]">{legendsComparison.legendB.nation}</p>
                              <p className="font-sans text-[10px] italic text-[#69707A] mt-2 block line-clamp-2">"{legendsComparison.legendB.quote}"</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="bg-[#121212] border border-white/5 p-4 rounded-none select-none">
                          <h4 className="font-mono text-[9px] tracking-widest text-[#69707A] uppercase border-b border-white/5 pb-2 mb-3.5 text-center">HISTORICAL BENCHMARK MATRIX</h4>
                          
                          <div className="space-y-3.5">
                            {/* World Cup Titles Comparison */}
                            <div>
                              <div className="flex justify-between text-xs font-mono mb-1">
                                <span className="text-[#AFA58D]">{legendsComparison.statsA.titles} Titles</span>
                                <span className="uppercase tracking-wider text-xs font-serif font-bold text-[#F5F2EA] py-0.5">WORLD CUP CHAMPIONSHIPS</span>
                                <span className="text-[#AFA58D]">{legendsComparison.statsB.titles} Titles</span>
                              </div>
                              <div className="w-full flex h-2.5 bg-[#1a1a1a] gap-0.5 rounded-none overflow-hidden">
                                <div 
                                  className="h-full bg-[#D4AF37] transition-all duration-300" 
                                  style={{ width: `${(legendsComparison.statsA.titles / (legendsComparison.statsA.titles + legendsComparison.statsB.titles || 1)) * 100}%` }}
                                />
                                <div 
                                  className="h-full bg-[#AFA58D]/40 transition-all duration-300"
                                  style={{ width: `${(legendsComparison.statsB.titles / (legendsComparison.statsA.titles + legendsComparison.statsB.titles || 1)) * 100}%` }}
                                />
                              </div>
                            </div>
                            ...
                            <div>
                              <div className="flex justify-between text-xs font-mono mb-1">
                                <span className="text-[#AFA58D]">{legendsComparison.statsA.goals} Goals</span>
                                <span className="uppercase tracking-wider text-xs font-serif font-bold text-[#F5F2EA] py-0.5">TOURNAMENT GOALS REGISTERED</span>
                                <span className="text-[#AFA58D]">{legendsComparison.statsB.goals} Goals</span>
                              </div>
                              <div className="w-full flex h-2.5 bg-[#1a1a1a] gap-0.5 rounded-none overflow-hidden">
                                <div 
                                  className="h-full bg-[#D4AF37] transition-all duration-300" 
                                  style={{ width: `${(legendsComparison.statsA.goals / (legendsComparison.statsA.goals + legendsComparison.statsB.goals || 1)) * 100}%` }}
                                />
                                <div 
                                  className="h-full bg-[#AFA58D]/40 transition-all duration-300"
                                  style={{ width: `${(legendsComparison.statsB.goals / (legendsComparison.statsA.goals + legendsComparison.statsB.goals || 1)) * 100}%` }}
                                />
                              </div>
                            </div>
                            ...
                            <div>
                              <div className="flex justify-between text-xs font-mono mb-1">
                                <span className="text-[#AFA58D]">Rating {legendsComparison.statsA.rating}</span>
                                <span className="uppercase tracking-wider text-xs font-serif font-bold text-[#F5F2EA] py-0.5">LEGACY RATING INDEX</span>
                                <span className="text-[#AFA58D]">Rating {legendsComparison.statsB.rating}</span>
                              </div>
                              <div className="w-full flex h-2.5 bg-[#1a1a1a] gap-0.5 rounded-none overflow-hidden">
                                <div 
                                  className="h-full bg-[#D4AF37] transition-all duration-300" 
                                  style={{ width: `${(legendsComparison.statsA.rating / (legendsComparison.statsA.rating + legendsComparison.statsB.rating || 1)) * 100}%` }}
                                />
                                <div 
                                  className="h-full bg-[#AFA58D]/40 transition-all duration-300"
                                  style={{ width: `${(legendsComparison.statsB.rating / (legendsComparison.statsA.rating + legendsComparison.statsB.rating || 1)) * 100}%` }}
                                />
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="bg-[#121212] border-l-2 border-[#D4AF37] p-4 text-xs font-sans leading-relaxed text-[#AFA58D] select-none">
                          <span className="font-mono text-[9px] uppercase tracking-widest text-[#D4AF37] block mb-1">CURATORIAL ANALYSIS DUEL VERDICT:</span>
                      ...
                      <div className="flex flex-col gap-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 select-none">
                          ...
                          >
                            <div>
                              <span className="font-mono text-[8px] opacity-75 uppercase tracking-wider block" style={{ color: nationsComparison.nationA.accentColor }}>NATION CIVILIZATION A</span>
                              <h4 className="font-serif text-lg font-bold tracking-wider leading-snug mt-1" style={{ color: nationsComparison.nationA.accentColor }}>{nationsComparison.nationA.name}</h4>
                              <p className="font-sans text-xs italic opacity-85 mt-2 block">"{nationsComparison.nationA.motto}"</p>
                            </div>
                            <div className="flex gap-4 border-t border-white/5 pt-3 mt-4 text-xs font-mono">
                              <div>🏆 {nationsComparison.nationA.titlesCount} Titles</div>
                              <div>🗺️ {nationsComparison.nationA.continent}</div>
                            </div>
                          </div>
                          ...
                          >
                            <div>
                              <span className="font-mono text-[8px] opacity-75 uppercase tracking-wider block" style={{ color: nationsComparison.nationB.accentColor }}>NATION CIVILIZATION B</span>
                              <h4 className="font-serif text-lg font-bold tracking-wider leading-snug mt-1" style={{ color: nationsComparison.nationB.accentColor }}>{nationsComparison.nationB.name}</h4>
                              <p className="font-sans text-xs italic opacity-85 mt-2 block">"{nationsComparison.nationB.motto}"</p>
                            </div>
                            <div className="flex gap-4 border-t border-white/5 pt-3 mt-4 text-xs font-mono">
                              <div>🏆 {nationsComparison.nationB.titlesCount} Titles</div>
                              <div>🗺️ {nationsComparison.nationB.continent}</div>
                            </div>
                          </div>
                        </div>
                        ...
                        {/* Dynamic Verdict */}
                        <div className="bg-[#121212] border-l-2 border-[#D4AF37] p-4 text-xs font-sans leading-relaxed text-[#AFA58D] select-none">
                          <span className="font-mono text-[9px] uppercase tracking-widest text-[#D4AF37] block mb-1">CURATORIAL SOUTH AMERICAN VS EUROPEAN CLASH REPORT:</span>
                  ...
                  <div className="bg-[#0b0b0b] border border-[#4E5661]/15 p-6 rounded-none shadow-xl">
                    {!quizActive ? (
                      <div className="text-center py-12 select-none">
                        ...
                        <h3 className="font-serif text-[#F5F2EA] text-xl tracking-wider uppercase font-bold mb-2">🏅 TRIVIA VAULT ARENA</h3>
                        <p className="font-sans text-xs text-[#AFA58D] max-w-md mx-auto leading-relaxed mb-8">
                          Validate your grasp of World Cup folklore, earn substantial Archive XP, and claim the coveted 'Trivia Medal' passport stamp.
                        </p>
                        <button
                      ...
                      <div className="select-none">
                        ...
                        <div className="flex items-center justify-between border-b border-white/5 pb-3.5 mb-5 font-mono text-[10px] text-[#69707A] uppercase tracking-widest">
                          <span>QUIZ PROGRESS: {currentIdx + 1} OF {quizQuestions.length}</span>
                          <span className="text-[#D4AF37]/80">ACCUMULATED SCORE: {score} / {currentIdx}</span>
                        </div>

                        {/* Question Panel */}
                        <div className="mb-6">
                          <span className="font-mono text-[9px] tracking-widest text-[#D4AF37] uppercase font-semibold">QUESTION CHOP {currentIdx + 1}</span>
                          <h4 className="font-serif text-[#F5F2EA] text-sm md:text-base font-bold leading-relaxed mt-1.5">{quizQuestions[currentIdx].question}</h4>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                          {quizQuestions[currentIdx].options.map((option, idx) => {
                            ...
                            return (
                              ...
                              >
                                <span>{option}</span>
                                {hasSubmitted && isCorrectAnswer && <CheckCircle2 className="w-4 h-4 text-emerald-400" />}
                                {hasSubmitted && isChosen && !isCorrectAnswer && <AlertCircle className="w-4 h-4 text-red-400" />}
                              </button>
                            );
                          })}
                        </div>
                        ...
                        {hasSubmitted && (
                          <div className="bg-[#121212] border border-white/5 p-4 rounded-none mb-6 animate-fade-in text-xs font-sans leading-relaxed text-[#AFA58D]">
                            <span className="font-mono text-[9px] uppercase tracking-widest text-[#D4AF37] block mb-1">HISTORICAL ANALYSIS ECHO:</span>
                        ...
                        {hasSubmitted && (
                          <div className="flex justify-end">
                            ...
                            >
                              {currentIdx + 1 === quizQuestions.length ? 'FINALIZE CHALLENGE' : 'NEXT HISTORIC FIELD →'}
                            </button>
                          </div>
                        )}
                      </div>
                ...
                {activeTab === 'profile' && (
                  <div className="bg-[#0b0b0b] border border-[#4E5661]/15 p-6 rounded-none shadow-xl flex flex-col gap-6">
                    <div className="flex justify-between items-center border-b border-white/5 pb-4 select-none">
                      <div>
                        <h3 className="font-serif text-[#F5F2EA] text-base uppercase tracking-widest font-bold">📜 ARCHIVE EXPLORER PASSPORT</h3>
                        <p className="font-sans text-xs text-[#69707A] leading-normal mt-0.5">Visually collect official custom stamps by exploring vaults and solving trivia.</p>
                      </div>
                      <button
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 select-none">
                      {stampDefs.map((stamp, i) => (
                        ...
                        >
                          ...
                          <div className={`w-14 h-14 mx-auto rounded-full border-2 border-dashed flex items-center justify-center text-xl mb-3.5 transition-all group-hover:scale-105 duration-300 ${stamp.color}`}>
                            {stamp.unlocked ? stamp.icon : '🔒'}
                          </div>

                          <div className="min-w-0">
                            <h4 className="font-serif text-[10px] tracking-widest uppercase text-[#F5F2EA] truncate font-bold leading-tight select-none">{stamp.title}</h4>
                            <p className="font-mono text-[8px] text-[#AFA58D] truncate select-none mt-1">{stamp.subtitle}</p>
                          </div>
                          ...
                          <div className="absolute inset-0 bg-[#060606]/95 opacity-0 group-hover:opacity-100 transition-opacity p-3 flex flex-col justify-center text-[9px] text-[#69707A] font-sans leading-relaxed text-center pointer-events-none">
                            <span className="font-mono text-[8px] text-[#D4AF37] uppercase tracking-wider font-semibold block mb-1">REQUIREMENT:</span>
                            {stamp.req}
                          </div>
                        </div>
                      ))}
